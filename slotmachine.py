...
def bid_time_limit(userCode, inputbid):
    ...
    user = filemanage.getUserInfo(userCode)
    usertime = user['ts']
    ...
    if str(nowtime) == usertime :
        if user['bid'] - inputbid < 0:
            ...
            filemanage.updateUserbid(userCode, inputbid)
            filemanage.updateUserMoney(userCode, inputbid)
            return "슬롯머신에게 {} 포인트를 빌렸습니다. \n".format(str(inputbid))
    # 다르다면
    else:
        filemanage.updateUserbid(userCode)
        filemanage.updateUserTime(userCode, nowtime)
        if user['bid'] - inputbid < 0 :
            return "일일 한도를 초과하는 액수입니다."
        else :
            filemanage.updateUserbid(userCode, inputbid)
...
def show_ranking():
    user_list = []
    temp = ""
    user_list = filemanage.show_top10_users()
    if len(user_list) == 0:
...
def play_main(text, slack_user_code):
    ...
    if cmd != "slot" or len(args) > 4:
        ...
        # 사용자를 등록합니다.# by jun
                ...
                ret = filemanage.addUser(slack_user_code)
                if ret == 0:
                    user = filemanage.getUserInfo(slack_user_code)
        ...
        # 사용자가 가진 돈이 얼마인지 보여줍니다.
            ...
            if filemanage.getUserInfo(slack_user_code) == None:
        ...
        # 사용자가 파산신청을 하여 슬롯머신에서 사용자를 삭제합니다.
            ...
            if filemanage.getUserInfo(slack_user_code) == None:
                result = '등록되지 않은 사용자입니다.'
                return result
            filemanage.delUser(slack_user_code)
            print("del user")
        ...
        # 슬롯머신을 돈을 걸고 돌립니다. 돈은 0 이상의 정수로 지정되야 합니다.
            ...
            if int(args[3]) > user['money']:
                ...
                filemanage.updateUserMoney(slack_user_code, -int(args[3]))
                if point == 0:
                    result = result + "안타깝습니다! \n"
                else :
                    result = result + "축하합니다! {}를 획득했습니다. \n".format(point)
                    filemanage.updateUserMoney(slack_user_code, point)
            user = filemanage.getUserInfo(slack_user_code)
            result = result + '<@{}> 님\n잔액 : {}     일일대출한도 : {}'.format(slack_user_code, user['money'], user['bid'])
